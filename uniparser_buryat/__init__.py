...
class BuryatAnalyzer(Analyzer):
    def __init__(self, mode='strict', verbose_grammar=False):
        ...
        if mode not in ('strict', 'nodiacritics'):
            return
        self.glossing = False
        self.dirName = 'uniparser_buryat.data_' + mode
    ...
    def analyze_words(self, words, format=None, disambiguate=False):
        ...
        """
        # if disambiguate:
        #     with as_file(files(self.dirName) / 'buryat_disambiguation.cg3') as cgFile:
...
if __name__ == '__main__':
    a = BuryatAnalyzer()
    # Check clitics
    analyses = a.analyze_words('түрүүлэгшэ')
    for ana in analyses:
        print(ana.wf, ana.lemma, ana.gramm, ana.gloss)
    analyses = a.analyze_words('түрүүлэгшэ', format='xml')
    print(analyses)
    analyses = a.analyze_words('түрүүлэгшэ', format='json')
    for ana in analyses:
        print(ana)
    ...
    analyses = a.analyze_words([['ба'], ['Би', 'шамдаа', 'дуратайб', '.']],
                               format='conll')
    print(analyses)
    analyses = a.analyze_words(['морфологи', [['ба'], ['Би', 'шамдаа', 'дуратайб', '.']]],
                               format='json')
    for ana in analyses:
        print(ana)
